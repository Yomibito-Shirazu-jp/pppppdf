...
import stirling.software.SPDF.service.PdfToIllustratorScriptService;
import stirling.software.common.annotations.AutoJobPostMapping;
import stirling.software.common.model.api.PDFFile;

/**
 * PDF → Adobe Illustrator ExtendScript (.jsx) 変換エンドポイント。
 * Gemini (Vertex AI) でPDF構造JSONをIllustratorスクリプトに変換する。
 */
@Slf4j
@RestController
...
public class PdfToIllustratorScriptController {

    private final PdfJsonConversionService pdfJsonConversionService;
    private final PdfToIllustratorScriptService pdfToIllustratorScriptService;
    ...
    public ResponseEntity<byte[]> convertPdfToIllustratorScript(
        ...
        if (!pdfToIllustratorScriptService.isAvailable()) {
        ...
        // Gemini でスクリプト生成
        String script = pdfToIllustratorScriptService.generateScript(pdfJson);
        ...
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION,
                        "attachment; filename=\"" + baseName + ".jsx\"")
