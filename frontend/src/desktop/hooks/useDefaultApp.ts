    ...
    try {
      ...
      if (result === 'set_successfully') {
        alert({
          ...
          body: t('defaultApp.success.message', 'B-PDF is now your default PDF editor'),
        });
        setIsDefault(true);
        ...
        alert({
          ...
          body: t('defaultApp.settingsOpened.message', 'Please select B-PDF in the file association dialogue'),
        });
      }
    } catch (error) {
